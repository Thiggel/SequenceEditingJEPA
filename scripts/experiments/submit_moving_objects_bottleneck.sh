...
RUN_SUFFIX="${RUN_SUFFIX:-bottleneck_v1_steps${MAX_STEPS:-5000}}"
    ...
    for seed in "${SEEDS[@]}"; do
      ...
      if [[ "${SUBMIT:-0}" == "1" ]]; then
        job_id="$(
          ...
          RUN_SUFFIX="${RUN_SUFFIX}" sbatch --parsable scripts/slurm/run_moving_objects_train.slurm
        )"
        printf '%s\t%s\t%s\t%s\t%s\n' "${run_name}" "${job_id}" "${latent_dim}" "${max_objects}" "${seed}" >> "${MANIFEST}"
      else
        printf 'DRY RUN: latent_dim=%s max_objects=%s seed=%s run=%s\n' \
...
if [[ "${SUBMIT:-0}" == "1" ]]; then
  ...
  printf 'Dry run only. Re-run with SUBMIT=1 to submit 90 single-CLS jobs.\n'
fi
