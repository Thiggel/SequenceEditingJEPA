...
- Check `puzzle_diag3d_reset_large`, `puzzle_diag3c_reset`, `puzzle_grid3b`, `puzzle_diag3b_large`, `puzzle_diag3b_n2`, and `puzzle_oversight` jobs first.
...
- Grid 3D reset-large confirmation was submitted as `3683903` with `scripts/slurm/run_grid3d_reset_large_diagnostics.slurm`. It is running against the rollout `N=2` checkpoint and writes `diagnostics_reset_cadence_large/` only at completion, comparing no reset, reset every 4/8 actions, and full re-encoded planning on 128 paired boards. As of `2026-05-31 13:30 CEST`, stderr is empty, stdout only has the task prologue, `sstat` shows CPU time close to elapsed with MaxRSS about `1.65 GiB`, and the output directory has not been created. Its 12-hour walltime ends at `2026-05-31 17:32:01 CEST`.
- Oversight `3683863` completed cleanly, `3684237` is the current run, and `3684889` is the only future oversight job, pending for `2026-05-31 17:27:28 CEST`.
- Do not start Maze, 10M/20M size sweeps, or broad controls yet. The next safe step is to analyze `3683903` when it completes or times out; if reset every 4 remains exact on the larger paired sample, pursue the planner-state reset/re-encoding branch before model scaling.
