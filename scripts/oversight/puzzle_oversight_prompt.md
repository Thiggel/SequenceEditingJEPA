...
- Do not start Maze, 10M/20M size sweeps, or broad controls yet. The next safe step is to implement the planner-state reset/re-encoding branch using symbolic candidate boards as the state of record and re-encoding latents every 4 actions for scoring. Keep the oracle-goal caveat explicit and add focused tests before submitting follow-up experiments.
