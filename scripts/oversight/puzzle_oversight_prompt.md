...
- Grid 3D reset-large confirmation was submitted as `3683903` with `scripts/slurm/run_grid3d_reset_large_diagnostics.slurm`. It is running against the rollout `N=2` checkpoint and writes `diagnostics_reset_cadence_large/`, comparing no reset, reset every 4/8 actions, and full re-encoded planning on 128 paired boards.
- Oversight `3682864` completed cleanly, `3683472` is the current run, and `3683863` is the only future oversight job, pending for `2026-05-31 09:27:03 CEST`.
- Do not start Maze, 10M/20M size sweeps, or broad controls yet. The next safe step is to analyze `3683903`; if reset every 4 remains exact on the larger paired sample, pursue the planner-state reset/re-encoding branch before model scaling.
