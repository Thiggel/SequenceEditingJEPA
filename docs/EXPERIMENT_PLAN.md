...
| Grid 4M hierarchical value L3 span-4 | Three-level hierarchy with `hierarchy_span=4`: terminal energy, primitive action advantage, discounted state value, and contrastive-margin energy. Each run evaluates flat learned-score reset, flat oracle reset, and level-2 oracle subgoal CEM; the three state-scorer variants also evaluate learned top-level subgoal CEM. | Running as `3711931_[0-3]` since 2026-06-10 11:42:19 CEST. Learned-score reset diagnostics solve `0/128` for all four variants; oracle latent-goal reset/calibration solves `128/128` with reset every 4 and re-encoded planning for all four. Jobs remain active in subgoal stages; no Grid 4M subgoal outputs yet as of 04:04 CEST. |
| Grid 4N macro-action advantage L3 span-4 | True macro-action advantage head on `(initial latent, current latent, continuous level-2 macro action)`, trained on oracle 16-step chunks and used as top-level subgoal CEM score. | Running as `3711983` since 2026-06-10 11:42:19 CEST. Oracle latent-goal reset/calibration finished: no-reset terminal-energy selection solved `127/128`, and reset every 4 plus re-encoded planning solved `128/128`. First latent-goal subgoal CEM output solves `0/32`, terminal `0/32`, mean remaining Hamming `48.47`; learned macro-action top-score output is still pending. |
...
| Grid 4P/4Q/4R replacement scheduled oversight | Replacement for the exact Europe/Berlin checks after stale watch `3715253` cancelled the first attempt. | `3715446` completed at 2026-06-10 18:18:46 CEST; `3715447` completed at 20:12:29 CEST; `3715448` completed at 00:14:48 CEST; `3715449` started at 04:00:06 CEST and confirmed proxy inheritance. `3715450` for 08:00 remains pending on `BeginTime`; none must submit successor oversight jobs. |
