...
| Grid 4U global single-latent MLP JEPA | Submitted as `3717900_[0-2]` at 2026-06-11 13:48:50 CEST; all three tasks are running on `a2141` under `rtxpro6k`. Training-only global-board MLP variants with `hierarchy_levels={1,2,3}`, one latent token for the whole Sudoku board, no attention, no CLS token, no cell marker, compact `action_embedding_dim=32`, and goal-energy regression. | Wrappers/configs: `scripts/slurm/run_grid4u_global_mlp_latent.slurm` and `configs/puzzle/grid4u_sudoku_global_mlp_l{1,2,3}.yaml`. This isolates whether a global bottlenecked board state learns a more useful scalar geometry than the token/attention encoder. Startup stderr files are empty; no results yet. |
