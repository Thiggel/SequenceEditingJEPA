...
| Grid 4M hierarchical value L3 span-4 | Three-level hierarchy with `hierarchy_span=4`: terminal energy, primitive action advantage, discounted state value, and contrastive-margin energy. Each run evaluates flat learned-score reset, flat oracle reset, and level-2 oracle subgoal CEM; the three state-scorer variants also evaluate learned top-level subgoal CEM. | Running as `3711931_[0-3]` since 2026-06-10 11:42:19 CEST; `_1` completed cleanly and `_0`, `_2`, `_3` still run. Learned-score reset diagnostics solve `0/128` for all four variants; oracle latent-goal reset/calibration solves `128/128` for all four. Latent-goal subgoal CEM now exists for all four variants and all solve `0/32`: terminal energy mean Hamming `49.44`, action advantage `49.25`, state value `48.75`, contrastive margin `49.59`. Learned top-score subgoal JSONs are still pending. |
...
| Grid 4Q recursive hierarchy planner on Grid 4M | New exact recursive planner diagnostic: level-2 optimizer proposes the first latent subgoal, level 1 recursively plans to that subgoal, and level 0 primitive CEM plans to the level-1 subgoal. Crosses top optimizer `{cem,gd,gd_reachability}` with Grid 4M methods. | Submitted as `3715252_[0-11]` with `afterok:3711931`; still dependency-blocked because Grid 4M is active in post-training diagnostics, no recursive artifacts yet as of 08:04 CEST. |
...
| Grid 4S HWM-style macro-action bottleneck/codebook | Submitted as `3717328_[0-9]` at 2026-06-11 11:17:31 CEST, then live array throttle increased from `%4` to `%10` by 11:30 CEST. All ten tasks are running: `_0`-`_3` on `a0536`, `_4`-`_7` on `a0934`, `_8` on `a0533`, and `_9` on `a0833`. Training-only L3 span-4 grid with macro dims `4/8/16/32`, VQ variants `(4,64)` and `(8,128)`, plus `state_value` and `macro_action_advantage` scorer variants. | Wrapper: `scripts/slurm/run_grid4s_macro_bottleneck_l3.slurm`. The previous hierarchy encoded action chunks, but macro actions lived at hidden width; this grid tests whether an HWM-style low-dimensional/codebook action manifold improves top-level reachability. All Grid 4S stderr files are empty; no results yet. |
| Grid 4T macro-bottleneck planner matrix | Submitted as dependent eval array `3717329_[0-57]` with `afterok:3717328`; pending on dependency. Eval-only array for Grid 4S checkpoints: flat symbolic MCTS to depth `32` with 256 simulations, recursive L1 hierarchy CEM, and recursive full L2/3-level hierarchy CEM. | Wrapper: `scripts/slurm/run_grid4t_macro_bottleneck_planner_eval.slurm`. Flat reset/beam is intentionally omitted. MCTS is treated as the level-0 far-lookahead diagnostic; recursive CEM is the main MPC-style hierarchy test. |
| Grid 4U global single-latent MLP JEPA | Completed cleanly as `3717900_[0-2]`: L1 elapsed `00:13:56`, L2 `00:24:43`, L3 `00:25:06`, all on `a2141`. Training-only global-board MLP variants with `hierarchy_levels={1,2,3}`, one latent token for the whole Sudoku board, no attention, no CLS token, no cell marker, compact `action_embedding_dim=32`, and goal-energy regression. | Wrappers/configs: `scripts/slurm/run_grid4u_global_mlp_latent.slurm` and `configs/puzzle/grid4u_sudoku_global_mlp_l{1,2,3}.yaml`. This isolates whether a global bottlenecked board state learns a more useful scalar geometry than the token/attention encoder. Stderr files are empty; planner results pending. |
| Grid 4V global single-latent planner matrix | Running as `3717901_[0-15%6]` after Grid 4U completed; tasks `_0`-`_5` started around 14:14 CEST and `_6`-`_15` are waiting behind the array throttle. Eval-only array for Grid 4U checkpoints: true receding-horizon MPC-CEM with learned `goal_energy` and oracle `latent_goal`, reset/beam controls with both scores, and recursive hierarchy CEM for L2/L3 with both top scores. | Wrapper: `scripts/slurm/run_grid4v_global_mlp_planner_eval.slurm`. MPC-CEM optimizes 16-step categorical symbolic action plans, executes the first action, re-encodes the symbolic board, and replans. Gate: compare global single-latent planning against prior tokenized JEPA behavior and check whether L2/L3 hierarchy helps or hurts. |
| Grid 4W global single-latent long-horizon MPC-CEM | Running as `3718124_[0-11%6]` after Grid 4U completed; tasks `_0`-`_5` started around 14:14 CEST and `_6`-`_11` are waiting behind the array throttle. Eval-only array for Grid 4U checkpoints: MPC-CEM horizons `32` and `64`, crossed with learned `goal_energy` and oracle `latent_goal`, for L1/L2/L3. | Wrapper: `scripts/slurm/run_grid4w_global_mlp_mpc_horizon_eval.slurm`. Same MPC-CEM budget as Grid 4V (`64` boards, population `128`, iterations `4`, execute `1`) except longer lookahead. Gate: check whether longer final-state projections make the learned or oracle scorer more separable than h16. |
| Grid 4X mixed rollout / mixed hierarchy global MLP | Submitted as `3718216_[0-3]` at 2026-06-11 14:30 CEST; all four tasks are running with empty startup stderrs. Training-only ablation: L1 mixed rollout K=2, L1 mixed rollout K=4, L2 mixed hierarchy span-4, and L3 mixed hierarchy span-4. Rollout/hierarchy trajectory batches use `oracle_probability=0.5`, so half are correct trajectories and half are coherent wrong/random trajectories. | Wrappers/configs: `scripts/slurm/run_grid4x_global_mlp_mixed_rollout.slurm` and `configs/puzzle/grid4x_sudoku_global_mlp_*`. This supersedes oracle-only hierarchy training as the next hierarchy read; existing 4U/4V/4W runs are kept as baselines. |
| Grid 4Y mixed rollout planner matrix | Submitted as dependent eval array `3718217_[0-19]` with `afterok:3718216`; pending on dependency. Eval-only array for Grid 4X checkpoints: MPC-CEM horizons `16/32` with learned `goal_energy` and oracle `latent_goal` for all four checkpoints, plus recursive hierarchy CEM for L2/L3 with both scores. | Wrapper: `scripts/slurm/run_grid4y_global_mlp_mixed_rollout_eval.slurm`. Gate: test whether off-policy wrong rollouts improve learned scoring or oracle dynamics under deeper MPC/hierarchy planning. |
