...
| Grid 4Z tokenized macro-bottleneck mixed-rollout rerun | Corrected job `3722524` is running on `rtxpro6k` node `a2143` with a 12h limit. First attempt `3722517` failed in 20s on Hydra append-override syntax before creating a run root. The fixed run reruns Grid 4S's best oracle-MCTS setting, terminal-energy L3 span-4 `macro_action_dim=4,VQ=64`, with `+training.rollout_oracle_probability=0.5` and `+training.hierarchy_oracle_probability=0.5`. | Wrapper: `scripts/slurm/run_grid4z_macro_bottleneck_mixed_l3.slurm`. Output root: `$PUZZLE_JEPA_WORK_ROOT/runs/sudoku_jepa_5m_hier_macro_bottleneck_l3_terminal_energy_d4_vq64_mixed_rollouts`. Gate: compare learned/oracle MCTS and recursive L2 CEM against Grid 4T's `d4_vq64` read. |
