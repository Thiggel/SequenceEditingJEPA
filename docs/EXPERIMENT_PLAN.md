...
| Grid 4M hierarchical value L3 span-4 | Three-level hierarchy with `hierarchy_span=4`: terminal energy, primitive action advantage, discounted state value, and contrastive-margin energy. Each run evaluates flat learned-score reset, flat oracle reset, and level-2 oracle subgoal CEM; the three state-scorer variants also evaluate learned top-level subgoal CEM. | Submitted as `3711931_[0-3]`; still pending at 2026-06-09 02:35 CEST due requested `a100_80` nodes reserved for maintenance. |
| Grid 4N macro-action advantage L3 span-4 | True macro-action advantage head on `(initial latent, current latent, continuous level-2 macro action)`, trained on oracle 16-step chunks and used as top-level subgoal CEM score. | Submitted as `3711983`; still pending at 2026-06-09 02:35 CEST due requested `a100_80` nodes reserved for maintenance. |
| Grid 4O inference-only MCTS value diagnostic | Existing original L1 goal-energy checkpoint, no training. MCTS uses exact symbolic Sudoku transitions, re-encodes leaf boards, and scores leaves with learned `goal_energy` or oracle `latent_goal`. Depths 8/16 test whether deeper tree search can rescue a locally flat scorer; oracle tasks control the MCTS implementation. | Submitted as `3714062_[0-3]`; running as of 2026-06-09 02:35 CEST on `a0632`. Third check found no MCTS artifacts yet. |
| Grid 4O one-shot oversight | User-requested non-recurring Codex checks for Grid 4O/4M/4N health, logs, results, bug fixes, analysis, and handoff updates. | `3714106` and `3714107` completed; `3714108` is running as the third and final requested check. Do not submit successors. |
