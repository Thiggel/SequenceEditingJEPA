...
| Grid 4M hierarchical value L3 span-4 | Three-level hierarchy with `hierarchy_span=4`: terminal energy, primitive action advantage, discounted state value, and contrastive-margin energy. Each run evaluates flat learned-score reset, flat oracle reset, and level-2 oracle subgoal CEM; the three state-scorer variants also evaluate learned top-level subgoal CEM. | Running as `3711931_[0-3]` since 2026-06-10 11:42:19 CEST; all variants reached step 5000 and wrote final `metrics.json`/`checkpoint.pt`; first post-training reset diagnostics still running, with empty diagnostic dirs and no reset/subgoal diagnostic JSON/JSONL as of 20:01 CEST. |
| Grid 4N macro-action advantage L3 span-4 | True macro-action advantage head on `(initial latent, current latent, continuous level-2 macro action)`, trained on oracle 16-step chunks and used as top-level subgoal CEM score. | Running as `3711983` since 2026-06-10 11:42:19 CEST; reached step 5000 and wrote final `metrics.json`/`checkpoint.pt`; first post-training oracle reset/calibration diagnostic still running, with an empty diagnostic dir and no reset/subgoal diagnostic JSON/JSONL as of 20:01 CEST. |
...
| Grid 4Q recursive hierarchy planner on Grid 4M | New exact recursive planner diagnostic: level-2 optimizer proposes the first latent subgoal, level 1 recursively plans to that subgoal, and level 0 primitive CEM plans to the level-1 subgoal. Crosses top optimizer `{cem,gd,gd_reachability}` with Grid 4M methods. | Submitted as `3715252_[0-11]` with `afterok:3711931`; still dependency-blocked because Grid 4M is active in post-training diagnostics, no artifacts yet as of 20:01 CEST. |
| Grid 4R recursive hierarchy planner on Grid 4N | Same recursive planner diagnostic for the true macro-action-advantage checkpoint, crossing `{cem,gd,gd_reachability}` with top score `macro_action_advantage`. | Submitted as `3715251_[0-2]` with `afterok:3711983`; still dependency-blocked because Grid 4N is active in post-training diagnostics, no artifacts yet as of 20:01 CEST. |
...
| Grid 4P/4Q/4R replacement scheduled oversight | Replacement for the exact Europe/Berlin checks after stale watch `3715253` cancelled the first attempt. | `3715446` completed at 2026-06-10 18:18:46 CEST, exit `0:0`; `3715447` started at 20:00:04 CEST and confirmed proxy inheritance; `3715448` for 2026-06-11 00:00, `3715449` for 04:00, and `3715450` for 08:00 remain pending on `BeginTime`; none must submit successor oversight jobs. |
