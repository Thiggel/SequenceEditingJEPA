...
| Grid 4S HWM-style macro-action bottleneck/codebook | Prepared but not submitted. Training-only L3 span-4 grid with macro dims `4/8/16/32`, VQ variants `(4,64)` and `(8,128)`, plus `state_value` and `macro_action_advantage` scorer variants. The hidden-width-ish `256` dim run was dropped. | Wrapper: `scripts/slurm/run_grid4s_macro_bottleneck_l3.slurm`. The previous hierarchy encoded action chunks, but macro actions lived at hidden width; this grid tests whether an HWM-style low-dimensional/codebook action manifold improves top-level reachability. |
| Grid 4T macro-bottleneck planner matrix | Prepared but not submitted. Eval-only array for Grid 4S checkpoints: flat symbolic MCTS to depth `32` with 256 simulations, recursive L1 hierarchy CEM, and recursive full L2/3-level hierarchy CEM. Each recursive level gets learned top score and oracle `latent_goal` controls. | Wrapper: `scripts/slurm/run_grid4t_macro_bottleneck_planner_eval.slurm`. Flat reset/beam is intentionally omitted. MCTS is treated as the level-0 far-lookahead diagnostic; recursive CEM is the main MPC-style hierarchy test. |
