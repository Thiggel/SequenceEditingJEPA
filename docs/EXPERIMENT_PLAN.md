...
| Grid 4Q recursive hierarchy planner on Grid 4M | New exact recursive planner diagnostic: level-2 optimizer proposes the first latent subgoal, level 1 recursively plans to that subgoal, and level 0 primitive CEM plans to the level-1 subgoal. Crosses top optimizer `{cem,gd,gd_reachability}` with Grid 4M methods. | Submitted as `3715252_[0-11]` with `afterok:3711931`; still dependency-blocked, no artifacts yet. |
| Grid 4R recursive hierarchy planner on Grid 4N | Same recursive planner diagnostic for the true macro-action-advantage checkpoint, crossing `{cem,gd,gd_reachability}` with top score `macro_action_advantage`. | Submitted as `3715251_[0-2]` with `afterok:3711983`; still dependency-blocked, no artifacts yet. |
...
| Grid 4P/4Q/4R replacement scheduled oversight | Replacement for the exact Europe/Berlin checks after stale watch `3715253` cancelled the first attempt. | Submitted as `3715446` for 2026-06-10 18:00, `3715447` for 20:00, `3715448` for 2026-06-11 00:00, `3715449` for 04:00, and `3715450` for 08:00; all are pending on `BeginTime` and must not submit successor oversight jobs. |
