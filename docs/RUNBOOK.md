...
| `3711931_[0-3]` | RUNNING | Grid 4M L3 span-4 hierarchical value ablation. Started at 2026-06-10 11:42:19 CEST on `a0632`/`a0633`; all variants reached step 5000 and wrote final checkpoints. Learned-score reset diagnostics solve `0/128` for every variant; reset-every-4 mean remaining Hamming is terminal energy `47.70`, action advantage `53.62`, state value `49.73`, contrastive margin `43.88`. Oracle latent-goal reset/calibration now solves `128/128` with reset every 4 and re-encoded planning for all four variants. Jobs remain active in subgoal stages; no Grid 4M subgoal outputs yet as of 04:04 CEST. |
| `3711983` | RUNNING | Grid 4N true macro-action advantage L3 span-4. Started at 2026-06-10 11:42:19 CEST on `a0633`; reached step 5000 and wrote final checkpoint. Oracle latent-goal reset/calibration finished: no-reset terminal-energy selection solved `127/128` with mean remaining Hamming `0.008`, and reset every 4 plus re-encoded planning solved `128/128`. First latent-goal subgoal CEM output solves `0/32`, terminal `0/32`, mean remaining Hamming `48.47`; learned macro-action top-score output is still pending. |
...
| `3715252_[0-11]` | PENDING | Grid 4Q recursive hierarchy diagnostics on Grid 4M checkpoints. Dependency `afterok:3711931`; still dependency-blocked because Grid 4M is active in post-training diagnostics. Crosses Grid 4M methods with optimizers `cem`, `gd`, `gd_reachability`. No recursive artifacts yet as of 04:04 CEST. |
| `3715251_[0-2]` | PENDING | Grid 4R recursive hierarchy diagnostics on Grid 4N macro-action checkpoint. Dependency `afterok:3711983`; still dependency-blocked because Grid 4N is active in post-training diagnostics. Crosses macro-action top score with optimizers `cem`, `gd`, `gd_reachability`. No recursive artifacts yet as of 04:04 CEST. |
...
| `3715446`; `3715447`; `3715448`; `3715449`; `3715450` | COMPLETED / RUNNING / PENDING | Exact-time one-shot oversight `3715446` completed at 2026-06-10 18:18:46 CEST, `3715447` completed at 20:12:29 CEST, and `3715448` completed at 2026-06-11 00:14:48 CEST. `3715449` started at 04:00:06 CEST on A40 node `a0321` and confirmed proxy inheritance; `3715450` remains begin-time-blocked for 08:00 CEST. They must not submit successor oversight jobs. |
