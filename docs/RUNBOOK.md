...
dimension and decodes/quantizes before predictor rollout. The prepared training
wrapper is `scripts/slurm/run_grid4s_macro_bottleneck_l3.slurm`, with L3
span-4 variants over macro dims `4/8/16/32`, VQ `(4,64)` and `(8,128)`, and
scorer variants `terminal_energy`, `state_value`, and
`macro_action_advantage`; the `256` dim run was dropped. The prepared eval
wrapper is `scripts/slurm/run_grid4t_macro_bottleneck_planner_eval.slurm`,
which runs flat MCTS depth `32`, recursive L1 hierarchy CEM, and recursive full
L2/3-level hierarchy CEM, with learned and oracle top-score controls where
applicable. Flat reset/beam is intentionally omitted.
...
| Grid 4S wrapper | PREPARED, not submitted | Training-only HWM-style L3 span-4 macro-action bottleneck/codebook grid in `scripts/slurm/run_grid4s_macro_bottleneck_l3.slurm`. It tests macro dims `4/8/16/32`, two VQ/codebook settings, and learned scorer variants after adding `model.macro_action_dim` and optional macro-action VQ. |
| Grid 4T wrapper | PREPARED, not submitted | Eval-only planner matrix for Grid 4S in `scripts/slurm/run_grid4t_macro_bottleneck_planner_eval.slurm`: flat MCTS depth `32`/256 sims, recursive L1 hierarchy CEM, and recursive full L2/3-level hierarchy CEM. Recursive runs include learned top score and oracle `latent_goal` controls. |
