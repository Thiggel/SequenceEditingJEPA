...
def run_grid5(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(1, max_steps + 1):
        model.train()
        batch = _sample_grid5_rollout_batch(
            world,
            train_examples,
            rng,
            batch_size,
        ...
        if step % save_every == 0 or step == max_steps:
            _save_grid5_checkpoint(output_dir / f"checkpoint-{step}.pt", model, optimizer, step, latest_metrics, config)
            _save_grid5_checkpoint(output_dir / "checkpoint.pt", model, optimizer, step, latest_metrics, config)

    (output_dir / "metrics.json").write_text(json.dumps(latest_metrics, indent=2, sort_keys=True))
    return latest_metrics


def _sample_grid5_rollout_batch(world, examples, rng, batch_size, steps, device, oracle_probability):
    rollouts = [
...
@torch.no_grad()
def _eval_grid5(model, world, examples, rng, train_cfg, eval_cfg, device) -> dict[str, Any]:
    batch_size = int(eval_cfg.get("batch_size", min(128, int(train_cfg["batch_size"]))))
    rollout_steps = int(train_cfg.get("rollout_steps", 8))
...
@hydra.main(version_base=None, config_path="../../configs/puzzle", config_name="grid5_sudoku_sigreg")
def main(cfg: DictConfig) -> None:
    config = OmegaConf.to_container(cfg, resolve=True)
    if not isinstance(config, dict):
        raise TypeError("Hydra config must resolve to a mapping.")
    print(json.dumps(run_grid5(config), indent=2, sort_keys=True), flush=True)
