...
def run_controlled_object_probes(
    ...
    *,
    ...
    return {
        "probe_schema": "controlled_objects_v2",
        **metrics,
    }
...
def _collect_labels(
    ...
    *,
    ...
    for _ in range(samples):
        trajectory = generator.sample_trajectory(rng, horizon=max_horizon)
...
def _encode_features(
    ...
    *,
    ...
    num_colors = model.encoder.color.num_embeddings
    for start in range(0, len(labels.states), batch_size):
        ...
        for level, span in enumerate(model.level_spans):
            rollout = latent
            for rollout_step in range(1, model.level_rollout_steps(level) + 1):
                action_start = (rollout_step - 1) * span
                action_stop = rollout_step * span
                rollout = model.predict_chunk(
                    level, rollout, actions[:, action_start:action_stop]
                )
                rollout_parts[f"level{level}_rollout{rollout_step}"].append(
                    rollout.detach()
                )
        raw = F.one_hot(states, num_classes=num_colors).flatten(1).float()
        raw_future = F.one_hot(futures, num_classes=num_colors).flatten(1).float()
...
def _fit_probe_suite(
    ...
    *,
    ...
    transform, predicted_transform = _fit_classifier(
        train.delta,
        train_y.actions[:, 0, 2],
        evaluate.delta,
        eval_y.actions[:, 0, 2],
    ...
    raw_transform, _ = _fit_classifier(
        train.raw_delta,
        train_y.actions[:, 0, 2],
        evaluate.raw_delta,
        eval_y.actions[:, 0, 2],
    ...
    metrics = {
        ...
        "probe_delta_transform_balanced_acc": transform,
        "probe_predicted_delta_transform_balanced_acc": predicted_transform,
        "raw_probe_delta_transform_balanced_acc": raw_transform,
